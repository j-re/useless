...
function getRequestedScopes(): string[] {
  const scopeQuery = getQueryParameter("scopes");
  if (!scopeQuery) {
    return defaultScopes;
  }

  const parsedScopes = scopeQuery
    .split(" ")
    .map((scope) => scope.trim())
    .filter((scope) => scope.length > 0);

  return parsedScopes.length > 0 ? parsedScopes : defaultScopes;
}
...
async function returnResult(publicClientApp: IPublicClientApplication, authResult: AuthenticationResult) {
  ...
  console.log("Sending auth result to parent window:", authResult);
...
export async function initializeMsal() {
  ...
  try {
    if (publicClientApp.getActiveAccount()) {
      const result = await publicClientApp.acquireTokenSilent(getTokenRequest(requestedScopes, false));
      if (result) {
        return returnResult(publicClientApp, result);
      }
    }
  ...
  publicClientApp.acquireTokenRedirect(getTokenRequest(requestedScopes, true, createLocalUrl("dialog.html")));
}
